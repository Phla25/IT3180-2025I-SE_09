...
public class TaiSanChungCuService {
    ...
    @Autowired
    private HoGiaDinhDAO hoGiaDinhDAO; // Cần để tìm HoGiaDinh khi liên kết
     ...
     * @return List<TaiSanChungCu>.
     */
    public List<TaiSanChungCu> getApartmentsByAreaRange(java.math.BigDecimal minDienTich,
            java.math.BigDecimal maxDienTich) {
     ...
     * @return List<TaiSanChungCu>.
     */
    public List<TaiSanChungCu> getApartmentsByValueRange(java.math.BigDecimal minGiaTri,
            java.math.BigDecimal maxGiaTri) {
     ...
     */
    public List<TaiSanChungCu> findApartmentsByFilters(BlueMoon.bluemoon.utils.AssetStatus status,
            java.math.BigDecimal minArea, java.math.BigDecimal maxArea, java.math.BigDecimal minValue,
            java.math.BigDecimal maxValue) {
        // Đây là cách đơn giản để xử lý lọc: nếu không có tham số nào, trả về tất cả.
        // Đối với logic lọc phức tạp hơn (kết hợp nhiều trường), cần viết JPQL động
        // trong DAO.

        // Vì DAO hiện tại chỉ có các hàm lọc đơn, ta sẽ cần thêm logic kết hợp trong
        // DAO hoặc Service,
        // nhưng tạm thời, ta sẽ chỉ hỗ trợ lọc đơn giản (hoặc kết hợp nếu cả min/max
        // đều có).
    ...
    /**
     * Lấy danh sách TẤT CẢ tài sản của một hộ gia đình (cho cư dân xem).
     * 
     * @param hoGiaDinh Hộ gia đình cần lấy tài sản.
     * @return List<TaiSanChungCu> danh sách tài sản thuộc hộ đó.
     */
    public List<TaiSanChungCu> getAssetsByHousehold(HoGiaDinh hoGiaDinh) {
        if (hoGiaDinh == null) {
            return new ArrayList<>();
        }
        return taiSanChungCuDAO.findAllAssetsByHousehold(hoGiaDinh);
    }
     ...
     * Thêm một Căn hộ mới.
     * 
     * @param canHoMoi Entity TaiSanChungCu chứa thông tin căn hộ (TenTaiSan,
     *                 DienTich, GiaTri, TrangThai, ViTri).
     * @param maHo     Mã Hộ gia đình liên kết (có thể là null hoặc chuỗi rỗng).
     * @return TaiSanChungCu đã được lưu.
     */
    @Transactional
    public TaiSanChungCu themCanHo(TaiSanChungCu canHoMoi, String maHo) {
        ...
        if (taiSanChungCuDAO.existsByTenCanHo(canHoMoi.getTenTaiSan())) {
            throw new IllegalArgumentException(
                    "Lỗi: Tên Căn hộ '" + canHoMoi.getTenTaiSan() + "' đã tồn tại. Vui lòng chọn tên khác.");
        }
        // 1. Kiểm tra và thiết lập Hộ gia đình liên kết
        if (maHo != null && !maHo.trim().isEmpty()) {
            HoGiaDinh hgd = hoGiaDinhDAO.findById(maHo)
            ...
            // Logic nghiệp vụ: Cần đảm bảo hộ này chưa sở hữu căn hộ chính nào khác nếu
            // cần.
     ...
     * @param maHo          Mã Hộ gia đình mới liên kết (có thể là null để gỡ liên
     *                      kết).
    ...
    public TaiSanChungCu capNhatCanHo(Integer maTaiSan, TaiSanChungCu canHoCapNhat, String maHo) {
        ...
        if (taiSanChungCuDAO.existsByTenCanHoExceptId(canHoCapNhat.getTenTaiSan(), maTaiSan)) {
            throw new IllegalArgumentException(
                    "Lỗi: Tên Căn hộ '" + canHoCapNhat.getTenTaiSan() + "' đã được sử dụng cho căn hộ khác.");
        }
    ...
    public void xoaCanHo(Integer maTaiSan) {
        ...
        // Thêm logic kiểm tra ràng buộc nghiệp vụ trước khi xóa (ví dụ: không được xóa
        // nếu có hóa đơn hoặc sự cố đang mở liên quan)
        // ... (Logic kiểm tra) ...
